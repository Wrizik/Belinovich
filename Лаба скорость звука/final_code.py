...
#теплоёмкости газов при постоянном давлении
Cp_O2 = 29.4
Cp_N2 = 29.125
Cp_Ar = 20.79
Cp_H2O = 33.26
...
#теплоёмкости газов при постоянном объёме
Cv_O2 = 20.79
Cv_N2 = 20.79
Cv_Ar = 12.47
Cv_H2O = 24.94
...
#открытие файлов с результатами измерений
...
#вычисление времени между регистрациями звуковой волны
def find_max(data):
    max_value = max(data)
...
print(f"скорость звука в чистом воздухе {L*nu/offset1} м/с")

#print(' ')

#(f"Максимум в файле 3: {max_value3} (Индекс: {max_index3})")
#print(f"Максимум в файле 4: {max_value4} (Индекс: {max_index4})")
#print(f"Смещение между максимумами: {offset2}")
print(f"скорость звука в выдыхаемом воздухе {L*nu/offset2} м/с")

V1 = L*nu/offset1
V2 = L*nu/offset2
def fun_chis(v2): # функция нахождения концентрации от скорости в атмосферном воздухе

    phi_чист = 0.379 #влажность в помещении в момент проведения измерений

    c_H2O_чист = phi_чист*3170/101375 #концентрация водяного пара

    #решение уравнения относительно концентрации углекислого газа
...
def fun_graz(v2): # для выдыхаемого воздуха, все аналогично скорости в атмосферном воздухе

    phi_грязн = 1 #влажность выдыхаемого воздуха
...
mas_x_chis=[i/10 for i in range(3000, 3500)]
mas_y_chis=[fun_chis(i**2) for i in mas_x_chis]
c_CO2_чист = fun_chis(V1**2)


#график выдыхаемого воздуха
mas_x_graz=[i/10 for i in range(3000, 3500)]
mas_y_graz=[fun_graz(i**2) for i in mas_x_graz]
c_CO2_грязн = fun_graz(V2**2)

print(' ')
print(f"содержание углекислого газа в чистом воздухе {c_CO2_чист*100} %")
print(f"содержание углекислого газа в выдохе {c_CO2_грязн*100} %")
